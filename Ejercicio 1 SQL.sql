...
select nombre_depto, avg(sal_emp) as promedio_salario
...
select nombre_depto, count(nombre_depto) as cantidad_empleados
from departamentos d , empleados e
where d.id_depto = e.id_depto
group by d.nombre_depto
having count(*)>3;
...
select nombre_depto, count(*) as cantidad_empleados
from departamentos d , empleados e
where d.id_depto = e.id_depto
group by d.nombre_depto;
-- having count(*)=0;
...
select nombre, sal_emp, (select round (avg(sal_emp))from personal.empleados) as sal_prom, nombre_depto
from departamentos d , empleados e
where d.id_depto = e.id_depto
and e.sal_emp > (select avg(sal_emp) from personal.empleados)order by d.nombre_depto;
